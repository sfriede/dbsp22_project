...
<?php
	...
	if($state1Valid == 1 && isset($population) && isset($poverty) && isset($unemployment) && isset($gdp) && isset($percentUnhoused) && isset($homelessness) && isset($income) && isset($foreignBorn) && isset($USBorn)) {

			   if ($stmt1 = $conn->prepare("CALL InsertEconomy(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
                           $stmt1->bind_param("sidddddddd", $state1, $population, $poverty, $unemployment, $gdp, $percentUnhoused, $homelessness, $income, $foreignBorn, $USBorn);
