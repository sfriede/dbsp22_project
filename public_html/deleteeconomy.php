...
<head><title>Delete Economy</title></head>
<body>
<?php
	...
	$state = $_POST['economyDelState'];
	
	if(isset($state)) {

			   if ($stmt1 = $conn->prepare("CALL DeleteEconomy(?)")) {
			      ...
			      //the result will contain whether or not such a state's record existed in the health table, so we can report that
                              ...
                              echo "Attempted to delete a record for ".$state." from the Economy Table";
